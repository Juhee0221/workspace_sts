...
# 지선영 #
    ...
    new_df.loc[(pd.to_numeric(new_df['소음(dB)']) > 40) & (pd.to_numeric(new_df['소음(dB)']) <= 60), '소음 등급'] = '보통'
    new_df.loc[(pd.to_numeric(new_df['소음(dB)']) > 60) & (pd.to_numeric(new_df['소음(dB)']) <= 80), '소음 등급'] = '나쁨'
    new_df.loc[(pd.to_numeric(new_df['소음(dB)']) > 80), '소음 등급'] = '매우나쁨'
# 윤상혁 #
    new_df['미세먼지 등급'] = '보통'

    new_df.loc[(pd.to_numeric(new_df['미세먼지 (㎍/㎥)']) >= 0) & (pd.to_numeric(new_df['미세먼지 (㎍/㎥)']) <= 30), '미세먼지 등급'] = '좋음'
    new_df.loc[(pd.to_numeric(new_df['미세먼지 (㎍/㎥)']) > 30) & (pd.to_numeric(new_df['미세먼지 (㎍/㎥)']) <= 80), '미세먼지 등급'] = '보통'
    new_df.loc[(pd.to_numeric(new_df['미세먼지 (㎍/㎥)']) > 80) & (pd.to_numeric(new_df['미세먼지 (㎍/㎥)']) <= 150), '미세먼지 등급'] = '나쁨'
    new_df.loc[pd.to_numeric(new_df['미세먼지 (㎍/㎥)']) > 150, '미세먼지 등급'] = '매우 나쁨'

    new_df['초미세먼지 등급'] = '보통'

    new_df.loc[(pd.to_numeric(new_df['초미세먼지 (㎍/㎥)']) >= 0) & (pd.to_numeric(new_df['초미세먼지 (㎍/㎥)']) <= 15), '초미세먼지 등급'] = '좋음'
    new_df.loc[(pd.to_numeric(new_df['초미세먼지 (㎍/㎥)']) > 15) & (pd.to_numeric(new_df['초미세먼지 (㎍/㎥)']) <= 35), '초미세먼지 등급'] = '보통'
    new_df.loc[(pd.to_numeric(new_df['초미세먼지 (㎍/㎥)']) > 35) & (pd.to_numeric(new_df['초미세먼지 (㎍/㎥)']) <= 75), '초미세먼지 등급'] = '나쁨'
    new_df.loc[pd.to_numeric(new_df['초미세먼지 (㎍/㎥)']) > 75, '초미세먼지 등급'] = '매우 나쁨'

    new_df['통합실내지수 등급'] = '보통'

    new_df.loc[(pd.to_numeric(new_df['통합실내지수']) >= 0) & (pd.to_numeric(new_df['통합실내지수']) < 50), '통합실내지수 등급'] = '매우 나쁨'
    new_df.loc[(pd.to_numeric(new_df['통합실내지수']) >= 50) & (pd.to_numeric(new_df['통합실내지수']) < 80), '통합실내지수 등급'] = '나쁨'
    new_df.loc[(pd.to_numeric(new_df['통합실내지수']) >= 80) & (pd.to_numeric(new_df['통합실내지수']) < 90), '통합실내지수 등급'] = '보통'
    new_df.loc[(pd.to_numeric(new_df['통합실내지수']) >= 90) & (pd.to_numeric(new_df['통합실내지수']) <= 100), '통합실내지수 등급'] = '좋음'

    # 이산화탄소의 평균
    new_df['이산화탄소 (ppm)'] = pd.to_numeric(new_df['이산화탄소 (ppm)']) # 새로 실행하면 실행해줘야 함
    avg_co2 = round(new_df['이산화탄소 (ppm)'].mean(), 2)

    # 이산화탄소 NaN 데이터에 평균 데이터 넣기
    new_df['이산화탄소 (ppm)'] = new_df['이산화탄소 (ppm)'].fillna(avg_co2)

    # 휘발성유기화합물의 평균
    new_df['휘발성유기화합물 (ppb)'] = pd.to_numeric(new_df['휘발성유기화합물 (ppb)']) # 새로 실행하면 실행해줘야 함
    avg_vocs = round(new_df['휘발성유기화합물 (ppb)'].mean(), 2)

    # 휘발성유기화합물 NaN 데이터에 평균 데이터 넣기
    new_df['휘발성유기화합물 (ppb)'] = new_df['휘발성유기화합물 (ppb)'].fillna(avg_vocs)
# 김주희 #
    new_df['온도(℃)'] = pd.to_numeric(new_df['온도(℃)'])
    new_df['습도(%)'] = pd.to_numeric(new_df['습도(%)'])
...
# INSERT START
    ...
    #
    SERIAL_NO = new_df['시리얼번호']
    DATA_TIME = new_df['데이터 시간']
    #
    PM_CODE = pd.to_numeric(new_df['미세먼지 (㎍/㎥)'])
    PM_GRADE = new_df['미세먼지 등급']
    FPM_CODE = pd.to_numeric(new_df['초미세먼지 (㎍/㎥)'])
    FPM_GRADE = new_df['초미세먼지 등급']
    CO2_CODE = pd.to_numeric(new_df['이산화탄소 (ppm)'])
    VOCS_CODE = pd.to_numeric(new_df['휘발성유기화합물 (ppb)'])
    CICI_CODE = pd.to_numeric(new_df['통합실내지수'])
    CICI_GRADE = new_df['통합실내지수 등급']

# 선영 INSERT #
    # DECIBEL_INFO 데이터 추가
    DECIBEL = pd.to_numeric(new_df['소음(dB)'])
    DECIBEL_GRADE = new_df['소음 등급']
    ...
    for i in range(len(new_df)):
        ...
        # 주희 #
        sql=f"INSERT INTO TEMANDHUM (DATE_TIME, SERIAL_NO, TEMPERATURE, HUMIDITY) VALUES ('{new_df['데이터 시간'][i]}', '{new_df['시리얼번호'][i]}', {new_df['온도(℃)'][i]}, {new_df['습도(%)'][i]})"
        #execute 실행
